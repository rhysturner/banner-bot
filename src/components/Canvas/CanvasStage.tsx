...
export default function CanvasStage() {
  ...
  const { scene, artboards, activeArtboardId, overrides, selectNode } =
    useEditor();
  ...
  useEffect(() => {
    ...
    c.on("selection:created", (e) =>
      selectNode((e.selected?.[0] as any)?.__nodeId ?? null),
    );
  ...
  // Sequence counter — incremented on every mount attempt so stale async
  // mounts can detect they've been superseded and abort.
  ...
  useEffect(() => {
    ...
    // When no CSV has been loaded there are no artboards — render the master
    // scene at its native dimensions so the imported file is always visible.
    const sized = ab ? resizeScene(scene, ab.width, ab.height) : scene;
    mountScene(
      c,
      sized,
      ab ? (overrides[ab.id] ?? {}) : {},
      () => seq !== mountSeqRef.current,
    );
